...
// Check @align is greater than 32
Test(sfmm_basecode_suite, memalign_align_too_small, .timeout = TEST_TIMEOUT) {
	sf_errno = 0;
	size_t size = 64, align = 8;
	void *ptr = sf_memalign(size, align);

	cr_assert_null(ptr, "ptr is not NULL!");
	cr_assert(sf_errno == EINVAL, "sf_errno is not EINVAL!");
	// We don't check free blocks because heap is never initialized
}

// Check @align is power of 2
Test(sfmm_basecode_suite, memalign_align_not_power_two, .timeout = TEST_TIMEOUT) {
	sf_errno = 0;
	size_t size = 200, align = 100;
	void *ptr = sf_memalign(size, align);
...
// Check ptr from sf_memalign is aligned to @align
Test(sfmm_basecode_suite, memalign_pp_alignment_check, .timeout = TEST_TIMEOUT) {
	size_t size = 64, align = 64;
	void *ptr = sf_memalign(size, align);

	cr_assert_not_null(ptr, "ptr is not NULL!");
	cr_assert(sf_errno != EINVAL, "sf_errno is EINVAL!");
	cr_assert(((unsigned long)ptr % align) == 0, "ptr is not correctly aligned");
}

Test(sfmm_basecode_suite, memalign_pp_alignment_check_bigger_parameters, .timeout = TEST_TIMEOUT) {
	size_t size = 10000, align = 32768;
	void *ptr = sf_memalign(size, align);
...
// Check @size is too big
Test(sfmm_basecode_suite, memalign_size_too_big, .timeout = TEST_TIMEOUT) {
	sf_errno = 0;
	// size is too big
	size_t size = 524288, align = 128;
	void *ptr = sf_memalign(size, align);
...
// Check sf_realloc when size is 0
Test(sfmm_basecode_suite, sf_realloc_size_zero, .timeout = TEST_TIMEOUT) {
	sf_errno = 0;
	void *malloc_ptr = sf_malloc(50);
	void *ptr = sf_realloc(malloc_ptr, 0);
	cr_assert_null(ptr, "ptr is not NULL!");
